 ...
 * 改类使用poetries数据库
 * 另一个使用poetries_l数据库*/
public class TtsDemo_Poetries extends AppCompatActivity implements View.OnClickListener{
    private static String TAG = TtsDemo.class.getSimpleName();
    public static final String POETRIES_ID_EXTRA="POETRIES_ID_EXTRA";
    public static final String PUSH_STORY_EXTRA="PUSH_STORY_EXTRA";
    // 语音合成对象
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isPush=getIntent().getBooleanExtra(TtsDemo.PUSH_STORY_EXTRA,false);
        getPeotryInfo(poet_id);
        //布局init
        initView();

        // 初始化合成对象
        mTts = SpeechSynthesizer.createSynthesizer(TtsDemo_Poetries.this, mTtsInitListener);
...
//				FlowerCollector.onEvent(TtsDemo.this, "tts_play");
            ...
            case R.id.comment_list:
                ...
                startActivity(new Intent(TtsDemo_Poetries.this, ListViewDemo.class));
    ...
    private void getPeotryInfo(int poet_id){
        String sql;
        if (isPush)
            sql="select p.title,l.name,p.content as poetries_content from poetries p ,poet_l l where p.id=" +
                    poet_id+" and p.poet_id=l.id";
        else
            sql="select pl.title,pl.name,pl.poetries_content from poetries pl where pl.id="+poet_id;
    ...
    Handler handler_for_poet=new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                ...
                Intent intent=new Intent(TtsDemo_Poetries.this, PoetryBrownser.class);
    ...
    protected void onResume() {
        //移动数据统计分析
        FlowerCollector.onResume(TtsDemo_Poetries.this);
    ...
    protected void onPause() {
        ...
        FlowerCollector.onPause(TtsDemo_Poetries.this);
        super.onPause();
    }
